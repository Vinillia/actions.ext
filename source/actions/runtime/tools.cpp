...
bool ActionsTools::LoadGameConfigFile(IGameConfig* config, char* error, size_t maxlen)
{
	...
	auto resolve_address = [config](const char* key, void** addr) -> bool
	{
	...
	if (!resolve_address("INextBot::DebugConColorMsg", &m_NextBot_DebugConColorMsg_Address))
	{
#if __DEBUG
	...
	if (!resolve_address("INextBot::IsDebugging", &m_NextBot_IsDebugging_Address))
	{
#if __DEBUG
...
void ActionsTools::InsertAction(
	...
	std::vector<nb_action_ptr>& actions)
{
	...
	InsertAction(action->GetActionBuriedUnderMe(), actions);
	InsertAction(action->GetActiveChildAction(), actions);
}