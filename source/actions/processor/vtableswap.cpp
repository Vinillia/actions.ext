...
{
	static constexpr inline size_t vtable_capacity = 200;
...
public:
	ProcessorVtableManager()
	{
		...
		if (vtable_capacity < vtable_contextual_query_size ||
			vtable_capacity < vtable_processor_size)
			{
				throw std::runtime_error("vtable size exceeds capacity");
			}
...
void ActionSwapManager::SwapAction(nb_action_ptr action, const ProcessorFunction* hf)
{
	SwitchFunction(action, hf, true);
}

void ActionSwapManager::UnSwapAction(nb_action_ptr action, const ProcessorFunction* hf)
{
...
void ActionSwapManager::SwitchFunction(nb_action_ptr action, const ProcessorFunction* hf, bool toOriginal)
{
	auto it = _original_vtable.find(action);
...
{
	while (!_original_vtable.empty())
	{
		StopActionProcessing(_original_vtable.begin()->first);
	}
}