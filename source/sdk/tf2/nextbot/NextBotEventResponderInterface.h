...
public:
	...
	virtual void OnSpokeConcept(CBaseCombatCharacter* who, AIConcept_t aiconcept, AI_Response* response);
	...
	virtual void OnActorEmoted(CBaseCombatCharacter* emoter, int emoteID);
...
inline void INextBotEventResponder::OnActorEmoted(CBaseCombatCharacter* emoter, int emoteID)
{
	for (INextBotEventResponder* sub = FirstContainedResponder(); sub; sub = NextContainedResponder(sub))
	{
		sub->OnActorEmoted(emoter, emoteID);
	}
}

inline void INextBotEventResponder::OnWin()
{
	for (INextBotEventResponder* sub = FirstContainedResponder(); sub; sub = NextContainedResponder(sub))
	{
		sub->OnWin();
...
inline void INextBotEventResponder::OnLose()
{
	for (INextBotEventResponder* sub = FirstContainedResponder(); sub; sub = NextContainedResponder(sub))
	{
		sub->OnLose();
